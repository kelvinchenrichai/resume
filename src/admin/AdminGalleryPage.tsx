import { Edit3, Eye, EyeOff, ImagePlus, Plus, Trash2, Upload, X } from 'lucide-react';
...
function GalleryEditor({ item, maxOrder, onSave, onClose }: { item: GalleryItem | null; maxOrder: number; onSave(item: GalleryItem | null, file: File | null, values: Record<string, string | boolean | number>): Promise<void>; onClose(): void }) {
  ...
  return <div className="modal-backdrop"><div className="modal gallery-editor">
    <div className="modal-head"><div><div className="eyebrow">{zh ? '相簿 CMS' : 'GALLERY CMS'}</div><h2>{item ? (zh ? '編輯相簿項目' : 'Edit gallery item') : (zh ? '新增相簿項目' : 'Add gallery item')}</h2></div><button onClick={onClose}><X/></button></div>
    <form className="editor-form" onSubmit={submit}>
      ...
      <div className="checks"><label><input name="isPublic" type="checkbox" defaultChecked={item?.isPublic ?? true}/>{zh ? '公開顯示' : 'Publish publicly'}</label></div>
...
export function AdminGalleryPage() {
  ...
  async function remove(item: GalleryItem) {
    ...
    try { await deleteGallery(item.id); setMessage(zh ? '相簿項目已刪除。' : 'Gallery item deleted.'); }
    catch (err) { setMessage(err instanceof Error ? err.message : zh ? '刪除失敗。' : 'Delete failed.'); }
  }

  return <div className="admin-page">
    <div className="admin-title"><div><div className="eyebrow">{zh ? '相簿 CMS' : 'GALLERY CMS'}</div><h1>{zh ? '相簿與證照' : 'Gallery & credentials'}</h1><p>{zh ? '上傳證照、獎項、活動照片與其他重要紀錄。' : 'Upload credentials, awards, event photos and milestones.'}</p></div><button className="button" onClick={() => setEditing(null)}><Plus size={16}/>{zh ? '新增相簿項目' : 'Add gallery item'}</button></div>
    {message && <div className="notice">{message}</div>}
    {sorted.length === 0 ? <div className="panel gallery-admin-empty"><ImagePlus size={35}/><h2>{zh ? '尚未上傳任何內容' : 'No gallery items yet'}</h2><p>{zh ? '按「新增相簿項目」上傳第一張證照或照片。' : 'Add your first credential or photo.'}</p></div> : <div className="admin-gallery-grid">
      {sorted.map((item) => <article className="admin-gallery-card" key={item.id}><img src={item.imageUrl} alt=""/><div><small>{item.categoryZh || item.category}{item.year ? ` · ${item.year}` : ''}</small><h3>{item.titleZh || item.title}</h3><span className={item.isPublic ? 'visibility public' : 'visibility'}>{item.isPublic ? <Eye size={14}/> : <EyeOff size={14}/>} {item.isPublic ? (zh ? '公開' : 'Public') : (zh ? '草稿' : 'Draft')}</span><div className="table-actions"><button onClick={() => setEditing(item)}><Edit3 size={16}/></button><button onClick={() => remove(item)}><Trash2 size={16}/></button></div></div></article>)}
    </div>}
